...
import com.example.jbtang.agi_4buffer.dao.configuration.ConfigurationDAO;
import com.example.jbtang.agi_4buffer.dao.configuration.ConfigurationDBManager;
import com.example.jbtang.agi_4buffer.dao.devices.DeviceDAO;
import com.example.jbtang.agi_4buffer.dao.devices.DeviceDBManager;
import com.example.jbtang.agi_4buffer.dao.logInfos.LogInfoManager;
...
public class MainMenuActivity extends AppCompatActivity {
    ...
    private static final int DEFAULT_SILENCECHECKTIME = 30;
    ...
    public void init() {
        ...
        saveToCache();
        saveToDAO();
        agi4bufferSP = getSharedPreferences(MAINMENU_DERECTORY, Context.MODE_PRIVATE);
        Context c = null;
        try {
        ...
        if(c!=null) {
            agi4SP = c.getSharedPreferences(MAINMENU_DERECTORY, Context.MODE_PRIVATE);
            if(!agi4SP.getString(TARGET_PHONE,"").equals(""))
                Global.Configuration.targetPhoneNum = agi4SP.getString(TARGET_PHONE,"");
        } else if(!agi4bufferSP.getString(TARGET_PHONE,"").equals("")){
            Global.Configuration.targetPhoneNum = agi4bufferSP.getString(TARGET_PHONE,"");
        }
        ...
        saveBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (phoneNum.getText().toString().toCharArray().length == 11) {
                    ...
                    saveToDAO();
                    SharedPreferences.Editor agi4Editor = agi4bufferSP.edit();
                    agi4Editor.putString(TARGET_PHONE, Global.Configuration.targetPhoneNum);
                    agi4Editor.commit();

                    if(agi4SP!=null) {
                        SharedPreferences.Editor agi4bufferEditor = agi4SP.edit();
                        agi4bufferEditor.putString(TARGET_PHONE, Global.Configuration.targetPhoneNum);
